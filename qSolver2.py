...
#Wavenumbers:
    ...
    psiAllTime = np.zeros((152,N,N2,2))
    ...
    for i in range( 1, ts+1):
        ...
        if i > lim:
            ...
            if cnt == 150:
                ...
                print(psiAllTime.shape)
                np.save(f'ICs/month_{cnt_month}.npy',psiAllTime)
                np.save(f'Averages/u1_month_{cnt_month}',ave_u1/cnt_ave)
                np.save(f'Averages/u2_month_{cnt_month}',ave_u2/cnt_ave)
                np.save(f'Averages/uv1_month_{cnt_month}',ave_uv1/cnt_ave)
                np.save(f'Averages/uv2_month_{cnt_month}',ave_uv2/cnt_ave)
                np.save(f'Averages/T1_month_{cnt_month}',ave_T1/cnt_ave)
                np.save(f'Averages/T2_month_{cnt_month}',ave_T2/cnt_ave)               
                print(f'==>writing data of the month {cnt_month}', flush=True)
                cnt = 0
        else:
            np.save(f'ICs/initials.npy',psi0)
    return
########################################################################################
...
T = (360*3)*(24/4.86) #days x (hours in 1 day / time scale)
