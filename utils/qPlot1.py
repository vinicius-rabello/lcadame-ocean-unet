...
def fourPlots(Lx,Ly,psi1,psi2,u1,u2,uv1,uv2,index):
    ...
    #plt.tight_layout()
    plt.tight_layout(rect=[0, 0.03, 0.5, 0.5])
    #plt.subplots_adjust(left=0.05, right=0.95, top=0.95, bottom=0.05, wspace=0.2, hspace=0.2)


    # Display the plots
    plt.savefig(f'images/fig_{index:0{6}}.png', dpi=300)
    #plt.show()
    plt.close()
...
num_iterations = 900#150*24 #355*150

with multiprocessing.Pool(processes=12) as pool:
    # Map the function to generate_and_save_plot to each index
    pool.map(task, range(num_iterations))
########################################################################
